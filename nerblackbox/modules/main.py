...
from nerblackbox.modules.utils.util_functions import get_run_name, compute_mean_and_dmean
...
class NerBlackBoxMain:
    ...
    def get_experiment_results(self):
        ...
        """
        from nerblackbox.modules.ner_training.lightning_ner_model_predict import LightningNerModelPredict
    ...
    def get_experiments_results(self):
        ...
        for _id in sorted(list(experiments_filtered.keys())):
            experiment_results = self._get_single_experiment_results(_id, verbose=self.verbose)
        ...
        df_single = pd.DataFrame(best_single_runs_overview) if self.as_df else best_single_runs_overview
        df_average = pd.DataFrame(best_average_runs_overview) if self.as_df else best_average_runs_overview
        if self.usage == 'cli':
    ...
    def predict(self):
        ...
        """
        nerbb = NerBlackBoxMain('get_experiment_results', experiment_name=self.experiment_name, usage='api')
    ...
    def show_experiment_config(self):
        ...
        path_experiment_config = join(env_variable('DIR_EXPERIMENT_CONFIGS'), f'{self.experiment_name}.ini')
    ...
    def show_experiment_configs():
        experiment_configs = glob.glob(join(env_variable('DIR_EXPERIMENT_CONFIGS'), '*.ini'))
        experiment_configs = [elem.split('/')[-1].strip('.ini') for elem in experiment_configs]
    ...
    def _create_data_directory():
        if resource_isdir(Requirement.parse('nerblackbox'), 'nerblackbox/modules/data'):
            data_source = resource_filename(Requirement.parse('nerblackbox'), 'nerblackbox/modules/data')
    ...
    def _get_single_experiment_results(self, experiment_id: str, verbose: bool = False):
        ...
        return ExperimentResults(_experiment, _single_runs, _average_runs, _best_single_run, _best_average_run)
