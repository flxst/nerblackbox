...
from nerblackbox.modules.utils.util_functions import \
    checkpoint2epoch, epoch2checkpoint, get_run_name, get_run_name_nr, compute_mean_and_dmean
...
class TestUtilFunctions:
    ...
    def test_checkpoint2epoch(self,
                              checkpoint_name: str,
                              epoch: int):
        ...
        assert test_checkpoint_name == checkpoint_name, f"ERROR! test_checkpoint_name = {test_checkpoint_name} != {checkpoint_name}"
    ...
    def test_get_run_name(self,
        ...
        assert test_run_name == run_name, f"ERROR! test_run_name = {test_run_name} != {run_name}"

        test_run_name_nr = get_run_name_nr(run_name, run_nr)
        assert test_run_name_nr == run_name_nr, f"ERROR! test_run_name_nr = {test_run_name_nr} != {run_name_nr}"
