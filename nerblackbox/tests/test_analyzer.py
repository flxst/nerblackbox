...
class TestAnalyzer:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                    "train",
                    4,
                    pd.DataFrame(data=[5, 3, 0, 0, 0, 0, 0],
                                 index=index,
                                 columns=["tags"]),
            ),
            (
                    "val",
                    2,
                    pd.DataFrame(data=[3, 1, 0, 0, 0, 0, 0],
                                 index=index,
                                 columns=["tags"]),
            ),
            (
    ...
    def test_get_tokens(self,
        ...
        assert test_num_tokens == num_tokens, \
        ...
        [
            (
                    ...
                    pd.DataFrame(
                        data={
                            "tags": [5, 3, 0, 0, 0, 0, 0],
                            "tags/sentence": ["{:.2f}".format(x) for x in [5 / 4., 3 / 4., 0, 0, 0, 0, 0]],
                            "tags relative w/ 0": ["{:.2f}".format(x) for x in [5 / 8., 3 / 8., 0, 0, 0, 0, 0]],
                            "tags relative w/o 0": ["{:.2f}".format(x) for x in [0, 3 / 3., 0, 0, 0, 0, 0]],
                        },
                        index=index,
                        columns=columns,
                    )
            ...
            (
                    ...
                    pd.DataFrame(
                        data={
                            "tags": [3, 1, 0, 0, 0, 0, 0],
                            "tags/sentence": ["{:.2f}".format(x) for x in [3 / 2., 1 / 2., 0, 0, 0, 0, 0]],
                            "tags relative w/ 0": ["{:.2f}".format(x) for x in [3 / 4., 1 / 4., 0, 0, 0, 0, 0]],
                            "tags relative w/o 0": ["{:.2f}".format(x) for x in [0, 1 / 1., 0, 0, 0, 0, 0]],
                        },
                        index=index,
                        columns=columns,
                    )
    ...
    def test_stats_aggregated_extend(self,
                                     phase: str,
                                     stats_aggregated_phase: pd.DataFrame,
                                     num_sentences: int,
                                     stats_aggregated_phase_extended: pd.DataFrame):
        test_stats_aggregated_phase_extended = \
            self.analyzer._stats_aggregated_extend(stats_aggregated_phase, num_sentences)
        pd.testing.assert_frame_equal(test_stats_aggregated_phase_extended, stats_aggregated_phase_extended), \
            ...
            (
                    {
                        ...
                        "total": pd.DataFrame(
                            data={
                                "tags": [11, 5, 0, 0, 0, 0, 0],
                                "tags/sentence": ["{:.2f}".format(x) for x in [11/8., 5/8., 0, 0, 0, 0, 0]],
                                "tags relative w/ 0": ["{:.2f}".format(x) for x in [11/16., 5/16., 0, 0, 0, 0, 0]],
                                "tags relative w/o 0": ["{:.2f}".format(x) for x in [0, 11/11., 0, 0, 0, 0, 0]],
                            },
                            index=index,
                            columns=columns,
                        ),
    ...
    def test_analyze_data(self,
        ...
        self.analyzer.analyze_data(write_log=False)  # created attr: stats_aggregated, num_tokens, num_sentences
        for phase in ["train", "val", "test", "total"]:
            ...
            pd.testing.assert_frame_equal(self.analyzer.stats_aggregated[phase], stats_aggregated[phase]), \
