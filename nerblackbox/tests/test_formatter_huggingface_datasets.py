...
class TestHuggingfaceDatasetsFormatter:
    ...
    formatter_pretokenized = HuggingfaceDatasetsFormatter("conll2003")     # use any ner_dataset that works
    formatter_unpretokenized = HuggingfaceDatasetsFormatter("ehealth_kd")  # use any ner_dataset that works
    ...
    def test_check_existence(self,
        ...
        assert test_existence == existence, \
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                    [
                        "swedish_ner_corpus", False  # only train & test on huggingface datasets
                    ]
            ),
            (
                    [
    ...
    def test_check_compatibility(self,
        ...
        assert test_compability == compatibility, \
    ...
    def test_check_implementation(self,
                                  ner_dataset: str,
                                  implementation: bool):
        test_implementation = HuggingfaceDatasetsFormatter.check_implementation(ner_dataset)
        assert test_implementation == implementation, \
    ...
    def test_get_info(self,
                      ner_dataset: str,
                      info: Tuple[bool, Optional[List[str]], Optional[bool], Optional[Dict[str, str]]]):
        print(ner_dataset)
        test_info = HuggingfaceDatasetsFormatter.get_infos(ner_dataset)
        print(test_info)
        for k in range(len(test_info)):
            assert test_info[k] == info[k], \
    ...
    def test_get_ner_tag_list(self,
                              ner_dataset: str,
                              ner_tag_list: List[str]):
        formatter = HuggingfaceDatasetsFormatter(ner_dataset)
        assert formatter.ner_tag_list == ner_tag_list, \
        ...
        [
            (
                    ...
                    [
                        ...
                        [['EU', 'B-ORG'], ['rejects', 'O'], ['German', 'B-MISC'], ['call', 'O'], ['to', 'O'],
                         ['boycott', 'O'], ['British', 'B-MISC'], ['lamb', 'O'], ['.', 'O']],
                    ]
            ),
            (
                    False,
                    [
                        {
                            ...
                            'tags': [
                                {'token': 'mayoría', 'tag': 'Predicate', 'char_start': 95, 'char_end': 102},
                                {'token': 'arritmias', 'tag': 'Concept', 'char_start': 110, 'char_end': 119},
                                {'token': 'problemas', 'tag': 'Action', 'char_start': 140, 'char_end': 149},
                                {'token': 'sistema eléctrico', 'tag': 'Concept', 'char_start': 156, 'char_end': 173},
                                {'token': 'corazón', 'tag': 'Concept', 'char_start': 178, 'char_end': 185}
                            ]
                        },
                        {
                            'text': 'En la leucemia linfocítica crónica, hay demasiados linfocitos, un tipo de glóbulos blancos.',
                            'tags': [
                                {'token': 'leucemia linfocítica crónica', 'tag': 'Concept', 'char_start': 6, 'char_end': 34},
                                {'token': 'linfocitos', 'tag': 'Concept', 'char_start': 51, 'char_end': 61},
                                {'token': 'glóbulos blancos', 'tag': 'Concept', 'char_start': 74, 'char_end': 90}
                            ]
                        }
                    ]
            ),
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                    [
                        pd.DataFrame(data=[["O O", "Mening 1"],
                                           ["PER O", "Mening 2"],
                                           ["O PER", "Mening 3"],
                                           ["O PER", "Mening 4"]]),
                        pd.DataFrame(data=[["O O", "Mening 5"],
                                           ["PER O", "Mening 6"]]),
                        pd.DataFrame(data=[["O O", "Mening 7"],
                                           ["PER O", "Mening 8"]]),
                    ]
            ),
        ]
    )
    def test_resplit_data(self,
        ...
        pd.testing.assert_frame_equal(test_df_test, df_test), \
