...
class TestAutoFormatter:
    ...
    def test_for_dataset(self,
        ...
        if error:
            ...
            assert isinstance(auto_formatter, BaseFormatter), \
                f"ERROR! type(auto_formatter) = {type(auto_formatter)} != BaseFormatter"
            assert auto_formatter.ner_dataset == ner_dataset, \
...
class TestBaseFormatter:
    ...
    def test_read_formatted_csvs(self,
                                 phases: List[str],
                                 df_formatted: pd.DataFrame):
        test_df_formatted = self.base_formatter._read_formatted_csvs(phases)
        pd.testing.assert_frame_equal(test_df_formatted, df_formatted), \
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                pd.DataFrame(data=[[1, 2], [3, 4], [5, 6], [7, 8]], columns=["A", "B"], index=["a", "b", "c", "d"]),
                0.25,
                pd.DataFrame(data=[[1, 2], [3, 4], [5, 6]], columns=["A", "B"], index=["a", "b", "c"]),
                pd.DataFrame(data=[[7, 8]], columns=["A", "B"], index=["d"]),
            )
        ]
    )
    def test_split_off_validation_set(self,
                                      df_original: pd.DataFrame,
                                      val_fraction: float,
                                      df_new: pd.DataFrame,
                                      df_val: pd.DataFrame):
        test_df_new, test_df_val = self.base_formatter._split_off_validation_set(df_original, val_fraction)
        pd.testing.assert_frame_equal(test_df_new, df_new), \
...
class TestAllFormatters:
    ...
    def test_create_ner_tag_mapping(self,
        ...
        assert isinstance(ner_tag_mapping, dict), \
